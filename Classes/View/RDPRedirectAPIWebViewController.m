...
- (void)initRDPRedirectAPIWrapper{
    ...
    api.api_mode = self.origin_type == RDPScanCard ? @"redirection_sop" : @"redirection_hosted";
    api.mid = self.mid;
    ...
    api.payer_name = self.payer_name;
    api.card_no = self.card_no;
    api.exp_date = self.exp_date;
    api.cvv2 = self.cvv2;
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    [RDPUIViewControllerExtension dismissPopUp:self];
...
- (void) webViewDidStartLoad:(UIWebView *)webView {
    [RDPUIViewControllerExtension showLoadingPopUp:self];
}
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if([request.URL.absoluteString containsString:self.redirectAPIWrapper.redirect_url]) {
        ...
        NSDictionary* returnParams = [RDPParameterExtractor extractParamFromURL:request.URL.absoluteString];
        ...
        NSString* signature = [RDPSignatureUtil makeRAPIGenericSignature:returnParams andSecretKey:self.secret_key];
        ...
        [[RDPHttpPostRequest alloc] initUsingHeader:NO andURL:url andData:data andResponseObject:[[RDPRedirectAPIRedirectionResponse alloc]init] andDelegate:self];
...
- (void) onSuccessWith:(RDPWsResponse*)responseObject {
    ...
    if([RDPSignatureUtil isRAPIGenericSignatureValidatedWithParams:responseObject.jsonObject andSecretKey:self.secret_key]) {
